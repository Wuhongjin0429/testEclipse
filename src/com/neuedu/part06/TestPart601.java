 ...
 * 
 * 类的创建过程
 * 先写属性：定义之后属性名变色，类中的方法里都可以直接使用（因为属性有默认值）String默认是null，int默认0
 ...
 * object类   是所有类的默认继承的父类
 * 
 * 构造方法
 * 是一种特殊的方法，用来初始化类的属性，方法名要与类名相同   不需要写返回值类型
 * [访问控制符] 类名([参数列表]){
 *     //方法体
 * }
 * 作用：
...
public class TestPart601 {
	...
	public static void main(String[] args) {
		...
		Employee epl2 = new Employee("李四",25,500);//通过调用有参的构造方法进行初始化
		System.out.println(epl2.showName()+":"+epl2.getSal());
		//epl2=null;//句柄丢失，引用无效
		
		Employee epl3 = new Employee("李泽言",18);
