...
public class TestPart1002 {
	...
	public static void main(String[] args) {
		...
		//map的key不能重复
		//key是由set来存放实现的（keyset），set的特点是无序且不能重复（通过hash算法）
		
		//遍历
		//1.根据key拿value
		Set<Integer> keySet = hashMap.keySet();//拿所有的key的集合
		
		Iterator<Integer> iterator = keySet.iterator();//调用迭代器方法
		...
		for (Entry<Integer, Integer> entry : entrySet) {//所有集合都具备foreach方法（加强for循环）
			System.out.println("key:"+entry.getKey()+",value:"+entry.getValue());
		}
		...
		while(true){
			...
			if("1".equals(type)){
				...
				while (iterator3.hasNext()) {
					...
					System.out.println("用户编号："+next.getKey()+" 姓名："+next.getValue().getName()
							+" 余额是："+next.getValue().getBalance());
				}
				...
				if(bankerInfo != null){
					System.out.println("用户编号："+bankerInfo.getNo()+" 姓名："+bankerInfo.getName()
					+" 余额是："+bankerInfo.getBalance());
				}else{
